...
try {
    class AIEngine {
        ...
        async init(triggerDownload = false) {
            ...
            const statusEl = (typeof document !== 'undefined') ? document.getElementById('ai-status') : null;
            if (statusEl) {
                ...
                this.embedder = await api.create({
                    monitor(m) {
                        m.addEventListener('downloadprogress', (e) => {
                            ...
                            if (typeof CustomEvent !== 'undefined' && globalScope.dispatchEvent) {
                                globalScope.dispatchEvent(new CustomEvent('ai-download-progress', { detail: { percentage: pct } }));
                            }
                        });
                    }
                });
    ...
    const globalScope = typeof window !== 'undefined' ? window : (typeof self !== 'undefined' ? self : globalThis);
    globalScope.linkyAIEngine = new AIEngine();
} catch (e) {
