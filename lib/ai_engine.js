...
try {
    class AIEngine {
        ...
        async summarizePage(text) {
            if (typeof Summarizer === 'undefined') return text.substring(0, 200);
            try {
                const availability = await Summarizer.availability();
                if (availability === 'no') return text.substring(0, 200);
                const summarizer = await Summarizer.create();
                return await summarizer.summarize(text);
            } catch (e) {
                console.warn("LinkStreak: Summarization failed:", e);
